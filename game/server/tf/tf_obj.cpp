...
{
	int iTargetHealth = m_bCarryDeploy ? m_iGoalHealth : GetMaxHealth();

	if ( GetHealth() < iTargetHealth )
	{
		int iAmountToHeal = min( 100, iTargetHealth - GetHealth() );
		...
		if ( iRepairCost > 0 )
		{
			...
			float flNewHealth = min( iTargetHealth, m_flHealth + ( iRepairCost * 5 ) );
