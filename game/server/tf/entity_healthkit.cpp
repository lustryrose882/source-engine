...
bool CHealthKit::MyTouch( CBasePlayer *pPlayer )
{
	...
	if ( ValidTouch( pPlayer ) )
	{
		...
		{
			int iDisguiseHealth = pTFPlayer->m_Shared.GetDisguiseHealth();
			int iDisguiseMaxHealth = pTFPlayer->m_Shared.GetDisguiseMaxHealth();
			int iHealthToAdd = ceil(iDisguiseMaxHealth * PackRatios[GetPowerupSize()]);

			iHealthToAdd = clamp(iHealthToAdd, 0, iDisguiseMaxHealth - iDisguiseHealth);

			pTFPlayer->m_Shared.SetDisguiseHealth( iDisguiseHealth + iHealthToAdd );
