	...
	if ( m_iTargetEntIndex )
	{
		...
		if ( pEnt )
		{
			if ( IsPlayerIndex( m_iTargetEntIndex ) )
			{
				...
				bReturn = ( pLocalTFPlayer->GetTeamNumber() == TEAM_SPECTATOR || pLocalTFPlayer->InSameTeam( pEnt ) || bDisguisedEnemy || (pLocalTFPlayer->IsPlayerClass( TF_CLASS_SPY ) && !pPlayer->m_Shared.InCond( TF_COND_STEALTHED )) );
			}
...
void CTargetID::UpdateID( void )
{
	...
	if ( m_iTargetEntIndex )
	{
		...
		m_pBGPanel->SetImage( m_pBGPanel->m_szTeamBG[ iTargetTeam ] );
